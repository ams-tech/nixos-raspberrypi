...
For boot-time consumers such as LUKS, set `neededForBoot = true`. This requires
`boot.initrd.systemd.enable = true`, writes only to `/run/...` paths, and keeps
the output owned by `root:root`. On the Raspberry Pi bootloader, installation
generates the persistent salt before initrd secrets are appended and fails early
if the OTP private key is not programmed.
